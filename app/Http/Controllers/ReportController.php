...
class ReportController extends Controller
{
            ...
            if($warehouse_id == 0) {
                if($product->is_variant) {
                    ...
                    foreach ($variant_id_all as $key => $variant_id) {
                        ...
                        $product_sale_data = Product_Sale::select('sale_unit_id', 'qty')->where([
                                                ['product_id', $product->id],
                                                ['variant_id', $variant_id]
                                        ])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->get();

                        $sold_qty = 0;
                        if(count($product_sale_data)) {
                    ...
                    $product_sale_data = Product_Sale::select('sale_unit_id', 'qty')->where('product_id', $product->id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->get();
        ...
        foreach ($lims_product_all as $product) {
            ...
            if($warehouse_id == 0){
                if($product->is_variant)
                    ...
                    $product_sale_data = Product_Sale::where('product_id', $product->id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->first();
            }
            else {
                if($product->is_variant)
    ...
    public function requisitionReport(Request $request)
    {
        ...
        $product_code = [];
        $product_qty = [];
        $lims_product_all = Product::select('id', 'code', 'name', 'qty', 'is_variant')->where('is_active', true)->get();
        
        foreach ($lims_product_all as $product) {
            $product_sale_data = null;
            
            $product_sale_data = Product_Sale::where('product_id', $product->id)->whereDate('created_at', '>=' , $start_date)
                                        ->whereDate('created_at', '<=' , $end_date)->first();  // need to change to sale date
        ...
        $product_warehouse = DB::table('product_warehouse')->select('warehouses.id', 'warehouses.name')
                            ->leftJoin('warehouses', 'warehouses.id', 'product_warehouse.warehouse_id')
                            ->whereIn('product_id', [$product_sale_data->product_id])->get();
        
        // dd($product_warehouse);
        return view('report.requisition',compact('product_id', 'product_name', 'product_code', 'product_qty', 'product_warehouse', 'start_date', 'end_date', 'warehouse_list','warehouse_id'));
    }
    ...
    public function warehouseReport(Request $request)
    {
        ...
        foreach ($lims_sale_data as $key => $sale) {
            $product_sale_data[$key] = Product_Sale::where('sale_id', $sale->id)->get();
        }
        ...
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        return view('report.warehouse_report', compact('warehouse_id', 'start_date', 'end_date', 'lims_purchase_data', 'lims_product_purchase_data', 'lims_sale_data', 'product_sale_data', 'lims_warehouse_list', 'lims_quotation_data', 'lims_product_quotation_data', 'lims_return_data', 'lims_product_return_data', 'lims_expense_data'));
    }
    ...
    public function userReport(Request $request)
    {
        ...
        return view('report.user_report', compact('lims_sale_data','user_id', 'start_date', 'end_date', 'product_sale_data', 'lims_payment_data', 'lims_user_list', 'lims_purchase_data', 'lims_product_purchase_data', 'lims_quotation_data', 'lims_product_quotation_data', 'lims_transfer_data', 'lims_product_transfer_data', 'lims_expense_data', 'lims_payroll_data') );
    }
    ...
    public function customerReport(Request $request)
    {
        ...
        return view('report.customer_report', compact('lims_sale_data','customer_id', 'start_date', 'end_date', 'product_sale_data', 'lims_payment_data', 'lims_customer_list', 'lims_quotation_data', 'lims_product_quotation_data', 'lims_return_data', 'lims_product_return_data'));
    }
